...
- **Technical Analysis & Strategies**: Built-in support for Moving Averages (SMA), RSI-based variants (RSI SMA, Two Bands, Rollercoaster), and scale-invariant logarithmic Mogalef Bands (`log_scale=True`) used as stop-loss levels for RSI band strategies, guaranteeing strictly positive lower bands and active stepping on decaying/high-volatility assets (see [Logarithmic Mogalef Bands Guide](docs/mogalef_bands_logarithmic_scale.md)). RSI Two Bands incorporates timeframe-adaptive 3-bar Daily Dwell Persistence to eliminate false-positive entry flickers (see [Daily Dwell Persistence Guide](docs/rsi_two_bands_daily_dwell_persistence.md)).
