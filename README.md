...
- **Technical Analysis & Strategies**: Built-in support for Moving Averages (SMA), RSI-based variants (RSI SMA, Two Bands, Rollercoaster), and scale-invariant logarithmic Mogalef Bands (`log_scale=True`) with timeframe-differentiated stop-loss clamping (Daily 12% / Weekly 18%) used as protective barriers for RSI band strategies, guaranteeing strictly positive lower bands, eliminating catastrophic tail losses, and preserving tighter corridors (see [Logarithmic Mogalef Bands Guide](docs/mogalef_bands_logarithmic_scale.md) and [Stop-Loss Clamping Guide](docs/timeframe_differentiated_stop_loss_clamping.md)). RSI Two Bands incorporates timeframe-adaptive 3-bar Daily Dwell Persistence to eliminate false-positive entry flickers (see [Daily Dwell Persistence Guide](docs/rsi_two_bands_daily_dwell_persistence.md)).
- **Modular Input Filtering**: Trade entry confirmation filters (True Range price action candle confirmation with overnight gap handling, ATR volatility regime, SMA 200 trend) with timeframe-level precomputed boolean masks and zero-allocation baseline bypass. Features asset-aware asymmetric filtering: standard assets remain unfiltered on Long positions and require downward confirmation on Shorts; inverse ETFs (`is_bear=True`) require strict upward candle confirmation ($\ge 0.85$) on Long setups and remain unfiltered on Shorts (see [RSI Strategies Input Filtering Guide](docs/rsi_strategies_input_filtering.md)).
...
- **Performance Metrics**: Detailed profiling including net profit, success rate, mathematical expectation (normalized percentage return), trade averages, and exposure (see [Trade Performance Percentage Ratios](docs/trade_performance_percentage_ratios.md)).
...
- **Database-Driven Shortability & Bear Asset Classification**: Dynamic resolution of long-only vs. short positions and inverse ETF (bear asset) asymmetric entry confirmation based on database security records.
...
For each symbol, `analyzer.py` downloads daily prices, derives weekly prices with Polars, and evaluates only the strategies enabled in `settings.yml`. `PriceProvider` translates internal symbols to Yahoo Finance tickers, auto-registers missing symbols into the database, and guards against empty downloads before converting Pandas data to Polars. Redundant downloads are eliminated via local Parquet caching (`PriceCache`) when eligible (see [Price Cache Guide](docs/local_price_cache.md)).

Database security records dynamically dictate execution scope: shortable status determines whether a symbol evaluates both long and short positions or long-only, while bear asset classification governs asymmetric input confirmation (standard assets trade longs unfiltered and confirm shorts; inverse ETFs trade long-only with upward candle confirmation). When `rsi_input_filter` is configured in `settings.yml` (e.g. `'price_action'`), trade eligibility masks are precomputed once per timeframe and injected into `RsiTwoBands` and `RsiRollerCoaster`, gating entries with zero runtime penalty during baseline runs (see [RSI Strategies Input Filtering Guide](docs/rsi_strategies_input_filtering.md)).

Shared RSI and scale-invariant logarithmic Mogalef Bands (`log_scale=True`) are calculated once per timeframe when RSI band strategies are active, providing guaranteed positive stop-loss channels (`LongStopLoss`, `ShortStopLoss`) that step dynamically even on decaying assets like `DUST` (see [Logarithmic Mogalef Bands Guide](docs/mogalef_bands_logarithmic_scale.md)). Stop-loss levels are vectorially clamped in Polars according to execution timeframe (Daily 12% / Weekly 18%) to eliminate catastrophic tail drawdowns while preserving tighter technical corridors (see [Stop-Loss Clamping Guide](docs/timeframe_differentiated_stop_loss_clamping.md)).

RSI Two Bands and Rollercoaster strategies retain `identify_old` for baseline comparison while `identify` runs the fused screening kernel leveraging shared inlined Numba scalar helpers. `RsiTwoBands` adaptively executes with 3-bar dwell persistence on `DAILY` evaluations to reject entry flickers and 2-bar baseline responsiveness on `WEEKLY` evaluations (see [Daily Dwell Persistence Guide](docs/rsi_two_bands_daily_dwell_persistence.md)). Strategy execution results (`Ratios`) and serialized current indicators (`sma`, `rsi`, `up`, `low`) are persisted transactionally in the database. 
