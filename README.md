...
- **Modular Input Filtering**: Trade entry confirmation filters (True Range price action candle confirmation with overnight gap handling, ATR volatility regime, SMA 200 trend) with timeframe-level precomputed boolean masks and zero-allocation baseline bypass (see [RSI Strategies Input Filtering Guide](docs/rsi_strategies_input_filtering.md)).
