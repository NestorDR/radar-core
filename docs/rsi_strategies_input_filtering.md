...
- If `net_profit > 0`, `expected_percentage > 0`, and `win_probability >= win_probability_threshold` (set via `settings.yml`, default `0.5`), the record is added to `positive_ratios_` and batch-upserted to the PostgreSQL `ratios` table.
